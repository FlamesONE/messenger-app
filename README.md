# Монорепо ТЗ (Nowkie) мессенджера

В данном репозитории реализована система мессенджера (как копия telegram в очень упрощенном виде) для тестового задания.

Реализован отдельно **backend** и **frontend** для возможной масштабируемости.

Сама система (что frontend, что backend) имеет довольно упрощенный вид как вариант для тестового задания, поэтому некоторые элементы специально упрощены (например, нет CSRF, нет защиты от XSS, упрощенная валидация файлов и т.п.).
...
```bash
git clone <repo-url> && cd Messanger
cp .env.example .env
```
...
│   ├── chat/                       #   Чаты
│   │   ├── chat.module.ts
│   │   ├── chat.http.ts            #     CRUD чатов
│   │   ├── chat.ws.ts              #     WebSocket события чатов
│   │   ├── dto/
│   │   └── use-cases/              #     create-chat, add-member, get-user-chats
│   ├── message/                    #   Сообщения
│   │   ├── message.module.ts
│   │   ├── message.http.ts         #     REST API сообщений
│   │   ├── message.ws.ts           #     Real-time обмен сообщениями
│   │   ├── dto/
│   │   └── use-cases/              #     send, delete, get-history, mark-as-read
...
│       ├── user.module.ts
│       ├── user.http.ts            #     GET/PATCH профиля
...
│   ├── ws.gateway.ts               #   WebSocket маршруты
