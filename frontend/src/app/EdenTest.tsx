      ...
      <Card className="w-full max-w-lg">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="max-h-64 overflow-y-auto rounded border bg-muted p-3 font-mono text-xs">
            {log.length === 0 && <span className="text-muted-foreground">Logs will appear here...</span>}
            {log.map((l, i) => (
              <div key={i} className={l.startsWith("ERROR") ? "text-destructive" : l.startsWith("OK") ? "text-green-600" : ""}>
                {l}
              </div>
            ))}
          </div>

          <Button variant="ghost" size="sm" onClick={() => setLog([])}>Clear logs</Button>
        </CardContent>
      </Card>
    </div>
  );
